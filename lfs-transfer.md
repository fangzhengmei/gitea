...
### 4.2 上传分块传输
- **触发点**: `services/lfs/server.go:507-508`
- 通过设置 `Transfer-Encoding: chunked` 头通知客户端支持分块上传
- 客户端 (git-lfs) 根据此头决定是否使用分块传输

### 4.3 客户端并发控制
- **配置**: `modules/setting/lfs.go:LFSClient` (L30-33)
- `BatchSize`: 每批处理的对象数 (默认 20)
- `BatchOperationConcurrency`: 并发传输数 (默认 8，与 git-lfs 默认 `lfs.concurrenttransfers` 一致)

```go
// modules/lfs/http_client.go:149-155
...
| `LFSClient.BATCH_SIZE` | 20 | 客户端每批处理对象数 |
| `LFSClient.BATCH_OPERATION_CONCURRENCY` | 8 | 客户端并发传输数 |
