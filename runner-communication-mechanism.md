...
- 令牌可重复使用，直到管理员生成新令牌时自动失效（`NewRunnerTokenWithValue` 会将同一作用域旧令牌标记为 `IsActive = false`）
- 注册令牌用于注册流程，注册成功后令牌仍保持激活状态，可继续用于注册更多运行器
...
**可验证的行为特征**：
- 当版本号相同时，不查询任务表，仅执行一次版本号查询
- 仅当版本号变化时，才执行完整的任务匹配逻辑
- 运行器每次 FetchTask 后更新本地缓存的版本号
...
2. **版本门控优化**：通过版本号过滤空轮询，版本相同时不查询任务表
